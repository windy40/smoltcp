...
impl InterfaceInner {
    ...
    pub(super) fn process_lorawan<'output, 'payload: 'output, T: AsRef<[u8]> + ?Sized>(
        &mut self,
        sockets: &mut SocketSet,
        schc_payload: &'payload T,
        _fragments: &'output mut FragmentsBuffer,
    ) -> Option<IpPacket<'output>> {
        //   let lorawan_frame = check!(LorawanFrame::new_checked(schc_payload));
        //   let lorawan_repr = check!(LorawanRepr::parse(&lorawan_frame));

        //    if lorawan_repr.frame_type != LorawanFrameType::Data {
        //        return None;
        //    }

        // match lorawan_frame.payload() {
        //     Some(payload) => self.process_schc(sockets, &lorawan_repr, payload, _fragments),
        //     None => None,
        // }
        self.process_ipv6(sockets, &check!(Ipv6Packet::new_checked(schc_payload)))
    }

    #[cfg(feature = "proto-schc")]
    pub(super) fn process_schc<'output, 'payload: 'output, T: AsRef<[u8]> + ?Sized>(
        &mut self,
        sockets: &mut SocketSet,
        lorawan_repr: &LorawanRepr,
        payload: &'payload T,
        f: &'output mut FragmentsBuffer,
    ) -> Option<IpPacket<'output>> {
        let payload = match check!(SchcPacket::dispatch(payload)) {
            #[cfg(not(feature = "proto-schc-fragmentation"))]
            SchcPacket::FragmentHeader => {
                net_debug!("Fragmentation is not supported, use the `proto-schc-fragmentation` feature to add support.");
                return None;
            }
            #[cfg(feature = "proto-schc-fragmentation")]
            SchcPacket::FragmentHeader => {
                match self.process_schc_fragment(lorawan_repr, payload, f) {
                    Some(payload) => payload,
                    None => return None,
                }
            }
            SchcPacket::IphcHeader => {
                match self.decompress_schc(
                    lorawan_repr,
                    payload.as_ref(),
                    None,
                    ...
                    Err(e) => {
                        net_debug!("schc decompress failed: {:?}", e);
                        return None;
                    }
                }
            }
    ...
    #[cfg(feature = "proto-schc-fragmentation")]
    fn process_schc_fragment<'output, 'payload: 'output, T: AsRef<[u8]> + ?Sized>(
        &mut self,
        lorawan_repr: &LorawanRepr,
        payload: &'payload T,
        f: &'output mut FragmentsBuffer,
        ...
        if frag.is_first_fragment() {
            ...
            if let Err(e) = frag_slot.add_with(0, |buffer| {
                self.decompress_schc(lorawan_repr, frag.payload(), Some(total_size), buffer)
        ...
        match frag_slot.assemble() {
            Some(payload) => {
                net_trace!("Schc: fragmented packet now complete");
                Some(payload)
            }
            None => None,
        }
    ...
    fn decompress_schc(
        ...
        let iphc = SchcIphcPacket::new_checked(iphc_payload)?;
        let iphc_repr = SchcIphcRepr::parse(
            &iphc,
            lorawan_repr.src_addr,
            lorawan_repr.dst_addr,
            &self.schc_address_context,
        )?;

        let mut decompressed_size = 40 + iphc.payload().len();

        let next_header = match iphc_repr.next_header {
            SchcNextHeader::Compressed => match SchcNhcPacket::dispatch(iphc.payload())? {
                SchcNhcPacket::ExtHeader => {
                    net_debug!("Extension headers are currently not supported for 6LoWPAN");
                    IpProtocol::Unknown(0)
                }
                SchcNhcPacket::UdpHeader => {
                    let udp_packet = SchcUdpNhcPacket::new_checked(iphc.payload())?;
                    let udp_repr = SchcUdpNhcRepr::parse(
                        &udp_packet,
                        &iphc_repr.src_addr,
                        &iphc_repr.dst_addr,
            ...
            SchcNextHeader::Uncompressed(proto) => proto,
        };

        if buffer.len() < decompressed_size {
            net_debug!("schc decompress: buffer too short");
        ...
        match iphc_repr.next_header {
            SchcNextHeader::Compressed => {
                match SchcNhcPacket::dispatch(iphc.payload())? {
                    SchcNhcPacket::ExtHeader => todo!(),
                    SchcNhcPacket::UdpHeader => {
    ...
    #[cfg(feature = "medium-lorawan")]
    pub(super) fn dispatch_lorawan<Tx: TxToken>(
        &mut self,
        //        ll_dst_a: LorawanAddress,  // not applicable to Lorawan
        tx_token: Tx,
        packet: IpPacket,
        frag: &mut Fragmenter,
    ) {
        ...
        // fragment it.
        //        let ll_src_a = self.hardware_addr.unwrap().lorawan_or_panic(); // not applicable to lorawan


        let ip_repr = packet.ip_repr();
        info!("[iface][dispatch] {:?}",ip_repr);
        ...
        let (src_addr, dst_addr) = match (ip_repr.src_addr(), ip_repr.dst_addr()) {
            ...
            _ => {
                net_debug!("dispatch_lorawan: dropping because src or dst addrs are not ipv6.");
                return;
            }
        };


        #[cfg(feature = "lorawan-packet")]
        // Create the IEEE802.15.4 header.
        let ieee_repr = LorawanRepr {
            frame_type: LorawanFrameType::Data,
            security_enabled: false,
            frame_pending: false,
            ack_request: false,
            sequence_number: Some(self.get_sequence_number()),
            pan_id_compression: true,
            frame_version: LorawanFrameVersion::Lorawan_2003,
        ...
        #[cfg(feature = "proto-schc")]
        {
            ...
            let iphc_repr = SchcIphcRepr {
                src_addr,
                ll_src_addr: Some(ll_src_a),
                dst_addr,
                ll_dst_addr: Some(ll_dst_a),
                next_header: match &packet {
                    IpPacket::Icmpv6(_) => SchcNextHeader::Uncompressed(IpProtocol::Icmpv6),
                    #[cfg(feature = "socket-tcp")]
                    IpPacket::Tcp(_) => SchcNextHeader::Uncompressed(IpProtocol::Tcp),
                    #[cfg(feature = "socket-udp")]
                    IpPacket::Udp(_) => SchcNextHeader::Compressed,
                    #[allow(unreachable_patterns)]
                    _ => {
                        net_debug!("dispatch_lorawan: dropping, unhandled protocol.");
                        return;
                    }
                },
            ...
            match packet {
                ...
                IpPacket::Udp((_, udpv6_repr, payload)) => {
                    let udp_repr = SchcUdpNhcRepr(udpv6_repr);
            ...
            if total_size + ieee_len > 125 {
                #[cfg(feature = "proto-schc-fragmentation")]
                {
                    // The packet does not fit in one Lorawan frame, so we need fragmentation.
                    ...
                    // call `dispatch_lorawan_frag`, which will transmit the other fragments.

                    // `dispatch_lorawan_frag` requires some information about the total packet size,
                    // the link local source and destination address...
                    let pkt = frag;

                    if pkt.buffer.len() < total_size {
                        net_debug!(
                                "dispatch_lorawan: dropping, fragmentation buffer is too small, at least {} needed",
                                total_size
                            );
                        return;
                    }

                    pkt.schc.ll_dst_addr = ll_dst_a;
                    pkt.schc.ll_src_addr = ll_src_a;

                    let mut iphc_packet =
                        SchcIphcPacket::new_unchecked(&mut pkt.buffer[..iphc_repr.buffer_len()]);
                    ...
                    match packet {
                        ...
                        IpPacket::Udp((_, udpv6_repr, payload)) => {
                            let udp_repr = SchcUdpNhcRepr(udpv6_repr);
                            let mut udp_packet = SchcUdpNhcPacket::new_unchecked(
                    ...
                    pkt.schc.datagram_size = (packet.ip_repr().payload_len() + 40) as u16;

                    // We generate a random tag.
                    let tag = self.get_schc_fragment_tag();
                    // We save the tag for the other fragments that will be created when calling `poll`
                    // multiple times.
                    pkt.schc.datagram_tag = tag;

                    let frag1 = SchcFragRepr::FirstFragment {
                        size: pkt.schc.datagram_size,
                        tag,
                    };
                    let fragn = SchcFragRepr::Fragment {
                        size: pkt.schc.datagram_size,
                        tag,
                        offset: 0,
                    };
                    ...
                    pkt.schc.fragn_size = (125 - ieee_len - fragn.buffer_len()) / 8 * 8;

                    pkt.sent_bytes = frag1_size;
                    pkt.schc.datagram_offset = frag1_size + header_diff;

                    tx_token.consume(ieee_len + frag1.buffer_len() + frag1_size, |mut tx_buf| {
                        // Add the IEEE header.
                        let mut ieee_packet = LorawanFrame::new_unchecked(&mut tx_buf[..ieee_len]);
                        ieee_repr.emit(&mut ieee_packet);
                        tx_buf = &mut tx_buf[ieee_len..];

                        // Add the first fragment header
                        let mut frag1_packet = SchcFragPacket::new_unchecked(&mut tx_buf);
                ...
                {
                    net_debug!(
                        "Enable the `proto-schc-fragmentation` feature for fragmentation support."
                    );
                    return;
                }
            } else {
                ...
                tx_token.consume(total_size + ieee_len, |mut tx_buf| {
                    ...
                    let mut iphc_packet =
                        SchcIphcPacket::new_unchecked(&mut tx_buf[..iphc_repr.buffer_len()]);
                    iphc_repr.emit(&mut iphc_packet);
        ...
        // hand over the ipv6 packet as is 
        let ip_len = ip_repr.buffer_len();
        tx_token.consume(ip_len, |mut tx_buf| {
            let mut ip_packet = Ipv6Packet::new_unchecked(&mut tx_buf[..ip_len]);
            
            if let IpRepr::Ipv6(ipv6_repr) = ip_repr {
//                net_trace!("[iface][tx_token emit]{:?}",ipv6_repr);
                ipv6_repr.emit(&mut ip_packet);
            }
        })
    }

    #[cfg(all(feature = "medium-lorawan", feature = "proto-schc-fragmentation"))]
    pub(super) fn dispatch_lorawan_frag<Tx: TxToken>(
        &mut self,
        tx_token: Tx,
        frag: &mut Fragmenter,
    ) {
        ...
        let ieee_repr = LorawanRepr {
            ...
            frame_version: LorawanFrameVersion::Lorawan_2003,
            dst_pan_id: self.pan_id,
            dst_addr: Some(frag.schc.ll_dst_addr),
            src_pan_id: self.pan_id,
            src_addr: Some(frag.schc.ll_src_addr),
        };

        // Create the FRAG_N header.
        let fragn = SchcFragRepr::Fragment {
            size: frag.schc.datagram_size,
            tag: frag.schc.datagram_tag,
            offset: (frag.schc.datagram_offset / 8) as u8,
        };

        let ieee_len = ieee_repr.buffer_len();
        let frag_size = (frag.packet_len - frag.sent_bytes).min(frag.schc.fragn_size);
        ...
        tx_token.consume(
            ...
            |mut tx_buf| {
                ...
                let mut frag_packet =
                    SchcFragPacket::new_unchecked(&mut tx_buf[..fragn.buffer_len()]);
                ...
                frag.schc.datagram_offset += frag_size;
            },
        );
    }
}
